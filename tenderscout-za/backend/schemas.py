...
class UserOut(_OrmBase):
    ...
    @field_validator("industry_preferences", "province_preferences",
                     "town_preferences", "municipality_preferences", mode="before")
    @classmethod
...
class SearchRequest(BaseModel):
    ...
    def page_positive(cls, v: int) -> int:
        if v < 1: raise ValueError("page must be >= 1")
        return v

    @field_validator("page_size")
    @classmethod
    def page_size_range(cls, v: int) -> int:
        if not (1 <= v <= 100): raise ValueError("page_size must be between 1 and 100")
        return v
    ...
    def radius_positive(cls, v: Optional[float]) -> Optional[float]:
        if v is not None and v <= 0: raise ValueError("radius_km must be positive")
        return v
