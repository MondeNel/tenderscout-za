...
def get_balance(current_user: models.User = Depends(auth_utils.get_current_user)):
    return {
        ...
        "rand_value": current_user.credit_balance * 10,
    }
...
def topup(
    ...
    # Convert to string to safely match dict keys
    pkg_key = str(request.package)
    if pkg_key not in PACKAGES:
        raise HTTPException(
            status_code=400, detail="Invalid package. Choose 100, 250, or 500"
        )

    credits = PACKAGES[pkg_key]
    ...
    db.add(
        models.Transaction(
            ...
            description=f"Top-up: R{request.package} — {credits} credits",
        )
    )
    db.commit()
