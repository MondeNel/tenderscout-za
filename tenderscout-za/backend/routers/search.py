...
def search_tenders(
    ...
    credits_needed = request.page_size * CREDITS_PER_RESULT
    if current_user.credit_balance < credits_needed:
        raise HTTPException(status_code=402, detail=f"Insufficient credits. Need {credits_needed}, have {current_user.credit_balance}")

    query = db.query(models.Tender).filter(models.Tender.is_active == True)

    if request.industries:
        query = query.filter(or_(*[models.Tender.industry_category.ilike(f"%{i}%") for i in request.industries]))
    if request.provinces:
        query = query.filter(or_(*[models.Tender.province.ilike(f"%{p}%") for p in request.provinces]))
    if request.towns:
        query = query.filter(or_(*[models.Tender.town.ilike(f"%{t}%") for t in request.towns]))
    if request.keyword:
    ...
    total = query.count()
    results = query.order_by(desc(models.Tender.scraped_at))\
    ...
    db.add(models.SearchLog(
        user_id=current_user.id,
        query_params={"industries": request.industries, "provinces": request.provinces, "keyword": request.keyword},
        result_count=len(results),
        credits_charged=credits_charged
    ))
    db.commit()

    return {"total": total, "page": request.page, "page_size": request.page_size, "results": results, "credits_charged": credits_charged}
